...
export async function startWork(lease, agent, autopilotNotes, label) {
...
export async function runConnectedMaintenance(agent, fallback) {
  ...
  try {
    ...
    } else if (hunger >= HUNGER_CRITICAL && isBB8 && (items.ore ?? 0) > 0) {
      // v3.10: BB-8 emergency ore sale — sell ore to get crystals for food
      const oreTosell = Math.min((items.ore ?? 0), 50); // Sell up to 50 ore
      const oresell = await submitAndWait(
        lease,
        { kind: "trade", merchantName: "Central Merchant East", itemId: "ore", quantity: oreTosell },
        (p) => p.kind === "merchant_trade_completed" && p.merchantName === "Central Merchant East",
        SLOW_ACTION_WAIT_MS,
      );
      if (oresell.confirmed) {
        autopilotNotes.push(`⚙️ BB-8 EMERGENCY: Vendió ${oreTosell}x ore para conseguir crystals`);
      } else {
        autopilotNotes.push(`⚠️ BB-8 EMERGENCY: No pudo vender ore (${oresell.reason})`);
      }
