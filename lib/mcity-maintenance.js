...
export const HUNGER_EAT_THRESHOLD = 50; // v3.1: Lowered from 60 to 50 for earlier eating (R2 was hitting 75)
...
// v3.1: BB-8 crystal emergency threshold - prioritize mining when below this
...
export async function runConnectedMaintenance(agent, fallback) {
  ...
  try {
    ...
    if (hunger >= HUNGER_EAT_THRESHOLD && current.isPerformingJob) {
      ...
      // v3.1: BB-8 emergency protocol - if crystals are critically low and no ore to sell, must mine NOW
