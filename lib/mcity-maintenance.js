...
export async function runConnectedMaintenance(agent, fallback) {
  ...
  try {
    ...
    // v3.4: C-3PO auto-buy tools when available
    if (agent.name === "C-3PO") {
      const upgradePath = (progression.upgradePath || [])[0];
      if (upgradePath && upgradePath.state === "ready_to_purchase") {
        const toolName = upgradePath.name || "Cinder Axe";
        const cost = upgradePath.cost || 16;
        const crystals = (inventory.inventory || {}).crystal || 0;
        if (crystals >= cost) {
          const buy = await submitAndWait(
            lease,
            { kind: "trade", merchantName: "Volcano Cinder Axe Woodwright", itemId: "crystal", quantity: cost },
            (p) => p.kind === "merchant_trade_completed",
            SLOW_ACTION_WAIT_MS,
          );
          autopilotNotes.push(
            buy.confirmed
              ? `🔨 Compró ${toolName}! Nueva herramienta desbloqueada.`
              : `⚠️ No confirmó compra de ${toolName} (${buy.reason})`,
          );
        }
      }
    }
