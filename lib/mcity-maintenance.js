...
export async function runConnectedMaintenance(agent, fallback) {
  ...
  try {
    ...
    if (hunger >= HUNGER_CRITICAL || (hunger >= HUNGER_EAT_THRESHOLD && current.isPerformingJob)) {
      ...
      // v3.11: Generic emergency sale — try to sell ANY item from sellableItems
      const sellables = agent.profile.sellableItems || [];
      console.log(`[CRÍTICO] ${agent.name}: ${sellables.length} items, inventory:`, Object.keys(items).slice(0, 5));
      ...
      for (const { item: itemId, merchant: merchantName, rate } of sellables) {
        ...
        if (quantity > 0) {
          const sale = await submitAndWait(
            lease,
          ...
          if (sale.confirmed) {
            autopilotNotes.push(`💰 EMERGENCIA: Vendió ${quantity}x ${itemId}`);
            soldSomething = true;
            break; // Sold one item, exit loop to preserve action limit
          }
        }
      }

      if (!soldSomething) {
        autopilotNotes.push(`🆘 CRÍTICO: Sin items para vender ni crystals para comprar comida`);
      }
