...
// v3.2: Two-tier hunger system (sep 25)
export const HUNGER_WORK_THRESHOLD = 60;     // Normal work range
export const HUNGER_EAT_THRESHOLD = 80;      // EMERGENCY: eat NOW (was 60, too low)
...
const SLOW_ACTION_WAIT_MS = 12_000; // may involve cross-district travel: trade, perform_job
const ACTION_POLL_MS = 1_000;
export const FOOD_ITEM_IDS = ["matcha_smoothie", "fish", "meat", "to_go_food"];

// v3.2: BB-8 crystal emergency + Aggressive wood selling
const BB8_CRYSTAL_EMERGENCY = 50;
const WOOD_SELL_THRESHOLD = 100; // C-3PO sells aggressively if carrying > 100 logs
...
const FOOD_BATCH_SIZE = 3; // smoothies per shopping trip
const CRYSTAL_BUFFER_TARGET = FOOD_BATCH_SIZE * FOOD_COST_CRYSTAL;
...
export async function runConnectedMaintenance(agent, fallback) {
  ...
  try {
    ...
    if (hunger >= HUNGER_CRITICAL || (hunger >= HUNGER_EAT_THRESHOLD && current.isPerformingJob)) {
      ...
      autopilotNotes.push(stop.confirmed ? "⏸️ Pausó el trabajo para comer" : "⚠️ No confirmó pausar el trabajo antes de comer");
      ...
      autopilotNotes.push(
        buy.confirmed
          ? `🛒 Compró ${batches}x ${FOOD_ITEM_ID} (come la próxima corrida)`
    ...
    } else if (isBB8 && crystals < BB8_CRYSTAL_EMERGENCY && sellableBatches === 0) {
      // v3.2: BB-8 emergency protocol - if crystals are critically low and no ore to sell, must mine NOW
      autopilotNotes.push(`🆘 BB-8 en emergencia: solo ${crystals} cristales. Prioriza MINING sobre cualquier otra acción.`);
      await startWork(lease, agent, autopilotNotes, "EMERGENCIA: Retomó mining");
    } else if (!isBB8 && agent.name === "C-3PO" && sellable > WOOD_SELL_THRESHOLD) {
      // v3.2: C-3PO aggressive wood selling - if carrying > 100 logs, sell aggressively
      ...
      autopilotNotes.push(
        sell.confirmed
          ? `🌳 C-3PO: Vendió ${quantity} madera (llevaba ${sellable}, agresivo)`
