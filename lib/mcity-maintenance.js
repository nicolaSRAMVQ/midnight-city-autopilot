...
export async function runConnectedMaintenance(agent, fallback) {
  ...
  try {
    ...
    if (hunger >= HUNGER_CRITICAL || (hunger >= HUNGER_EAT_THRESHOLD && current.isPerformingJob)) {
      ...
      // v3.10: CRITICAL hunger fix — eat REGARDLESS of pause success
      if (hunger >= HUNGER_CRITICAL && hasFood) {
        const eat = await submitAndWait(lease, { kind: "eat" }, (p) => p.kind === "agent_ate", FAST_ACTION_WAIT_MS);
        autopilotNotes.push(
          eat.confirmed
            ? `🍽️ CRÍTICO: Comió ${eat.payload.itemId} (hambre ${eat.payload.hungerBefore}→${eat.payload.hungerAfter})`
            : `⚠️ CRÍTICO: No pudo comer (${eat.reason ?? "sin datos"})`,
        );
      }
